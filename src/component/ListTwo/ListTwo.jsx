...
import { setLocalTwo } from "../../store/slice/smartSlice";
...
function ListTwo() {
  ...
  const { smart, smartTwo } = useSelector((state) => state.smart);
  ...
  function saveSmart() {
    ...
    const res = smartStateTwo.filter((el) => el.id !== newSmart.id);
    res.push(newSmart);
    res.sort((a, b) => a.id - b.id);
    dispatch(setLocalTwo(res));
    handleClose();
  }
  ...
  return (
    <div className="list-two">
      ...
      <h3 className="list-two-title">Түштөн кийинки сабактар</h3>
      <table
        style={{
          background: "white",
